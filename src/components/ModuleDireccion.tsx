...
export default function ModuleDireccion({
  ...
  const [sheetsUrl, setSheetsUrl] = useState<string>(() => {
    return localStorage.getItem('f_estudio_google_sheets_url') || '';
  });
  ...
  const monthlyAggregates = useMemo(() => {
    ...
    // Seed standard baseline history to give a rich experience even with empty state
    const baseMonths = ['Abril 2026', 'Mayo 2026', 'Junio 2026', 'Julio 2026'];
    const baseGross: Record<string, number> = { 'Abril 2026': 90000, 'Mayo 2026': 105000, 'Junio 2026': 195000, 'Julio 2026': 125000 };
    const baseExpenses: Record<string, number> = { 'Abril 2026': 58000, 'Mayo 2026': 62000, 'Junio 2026': 74800, 'Julio 2026': 68400 };

    baseMonths.forEach(m => {
      aggregates[m] = {
        gross: baseGross[m],
        expenses: baseExpenses[m],
        net: baseGross[m] - baseExpenses[m],
        transactionList: []
      };
    });
  ...
  const topActivities = useMemo(() => {
    ...
    // Default items fallback if database is empty or no time registered yet
    if (items.length === 0) {
      return [
        { name: 'Estructura Metálica Reforzada', code: 'PED-001', actualHours: 42, avgProgress: 100 },
        { name: 'Gabinete Portarrodillos Industrial', code: 'PED-002', actualHours: 35, avgProgress: 80 },
        { name: 'Soporte de Carga Angular', code: 'PED-003', actualHours: 24, avgProgress: 60 },
        { name: 'Parrilla de Enfriamiento Especial', code: 'PED-004', actualHours: 18, avgProgress: 100 }
      ];
    }
    return items;
  }, [productionTasks]);
  ...
  const handleSaveSheetsUrl = (url: string) => {
    setSheetsUrl(url);
    localStorage.setItem('f_estudio_google_sheets_url', url);
    setSyncStatus('idle');
    setSyncMessage('URL configurada. Listo para sincronizar.');
  };

  // Google Apps Script copy-paste generator code
  const googleAppsScriptCode = `/**
 * =========================================================================
 * GOOGLE APPS SCRIPT PARA SINCRONIZACIÓN CLOUD - ERP F. ESTUDIO
 * =========================================================================
 * 
 * Sigue estas instrucciones paso a paso para conectar tu ERP con la nube:
 * 
 * 1. Abre Google Sheets (una hoja de cálculo nueva o existente).
 * 2. En el menú superior, ve a: "Extensiones" > "Apps Script".
 * 3. Borra todo el código que aparezca por defecto en el editor.
 * 4. Pega este código completo en el editor de Apps Script.
 * 5. Haz clic en el botón de Guardar (icono de disco).
 * 6. Haz clic en "Implementar" > "Nueva implementación" (botón azul arriba).
 * 7. En el engranaje de "Seleccionar tipo", elige "Aplicación web".
 * 8. Configura los parámetros:
 *    - Descripción: Sincronizador ERP F. Estudio
 *    - Ejecutar como: "Yo" (tu correo de Google)
 *    - Quién tiene acceso: "Cualquiera" (necesario para habilitar la API pública)
 * 9. Haz clic en "Implementar" y autoriza los permisos de Google.
 * 10. Copia la "URL de la aplicación web" generada (debe terminar en "/exec").
 * 11. Pégala en el panel de control del Módulo 1 en el ERP y pulsa Sincronizar.
 */
...
function doPost(e) {
  try {
    ...
    // Almacenamos el backup JSON principal para habilitar la carga en otros dispositivos
    scriptProperties.setProperty("F_ESTUDIO_STATE_BACKUP", JSON.stringify(contents));
    
    var ss = SpreadsheetApp.getActiveSpreadsheet();
    
    // Escribir hojas tabulares legibles para humanos automáticamente
    if (contents.transactions) populateTabularSheet(ss, "Transacciones_Finanzas", [
      "ID", "Tipo", "Concepto", "Monto", "Fecha", "Categoría"
    ], contents.transactions, function(row) {
      return [row.id, row.type === "income" ? "Ingreso" : "Egreso", row.concept, row.amount, row.date, row.category];
    });
    
    if (contents.orders) populateTabularSheet(ss, "Pedidos_Ventas", [
      "ID Pedido", "Cliente", "Producto", "Cantidad", "Precio Unitario", "Total", "Fecha Pedido", "Fecha Entrega", "Estado"
    ], contents.orders, function(row) {
      return [row.id, row.clientName, row.productName, row.quantity, row.unitPrice, row.total, row.date, row.deliveryDate, row.status];
    });
    
    if (contents.fichasTecnicas) populateTabularSheet(ss, "Fichas_Tecnicas", [
      "Código", "Nombre", "Descripción", "Dimensiones", "Horas Estimadas", "Dificultad"
    ], contents.fichasTecnicas, function(row) {
      return [row.code, row.name, row.description, row.dimensions, row.estimatedHours, row.difficulty];
    });
    
    if (contents.productionTasks) populateTabularSheet(ss, "Taller_Produccion", [
      "ID Tarea", "Pedido", "Pieza/Producto", "Asignado A", "Progreso %", "Estado", "Última Actualización"
    ], contents.productionTasks, function(row) {
      return [row.id, row.orderId, row.productName, row.assignedTo, row.progress, row.status, row.updatedAt];
    });

    if (contents.okrs) populateTabularSheet(ss, "OKRs_Estrategia", [
      "Objetivo", "Resultado Clave", "Meta", "Actual", "Avance %", "Unidad"
    ], contents.okrs, function(row) {
      var progress = row.target > 0 ? Math.round((row.current / row.target) * 100) : 0;
      return [row.objective, row.keyResult, row.target, row.current, progress, row.unit];
    });
    
    return ContentService.createTextOutput(JSON.stringify({ 
      status: "success", 
      message: "¡ERP sincronizado con Google Sheets con éxito! Tablas actualizadas." 
...
function doGet() {
  try {
    var scriptProperties = PropertiesService.getScriptProperties();
    var backup = scriptProperties.getProperty("F_ESTUDIO_STATE_BACKUP");
    if (!backup) {
      return ContentService.createTextOutput(JSON.stringify({ 
        status: "empty", 
    ...
    return ContentService.createTextOutput(backup).setMimeType(ContentService.MimeType.JSON);
  } catch(error) {
    return ContentService.createTextOutput(JSON.stringify({ 
      status: "error", 
      message: error.toString() 
...
// Función auxiliar para rellenar datos formateados en la hoja de cálculo
function populateTabularSheet(ss, sheetName, headers, dataArray, rowMapper) {
  ...
  // Darle un estilo premium a la cabecera
  var headerRange = sheet.getRange(1, 1, 1, headers.length);
  headerRange.setBackground("#0f172a"); // Charcoal
  ...
  if (dataArray && dataArray.length > 0) {
    var rows = dataArray.map(rowMapper);
  ...
  // Copies Apps Script code to clipboard
  ...
  // Simulates cloud synchronization via Webhook/Apps Script API
  const handleSyncCloud = async () => {
    if (!sheetsUrl) {
      setSyncStatus('error');
      setSyncMessage('Por favor introduce primero una URL válida de Google Apps Script Web App.');
      return;
    }

    setSyncStatus('syncing');
    setSyncMessage('Estableciendo puente y transfiriendo bases de datos del ERP...');

    try {
      // Gather full ERP datasets to push
      const payload = {
        fichasTecnicas: JSON.parse(localStorage.getItem('f_estudio_db_fichas') || '[]'),
        orders: JSON.parse(localStorage.getItem('f_estudio_db_orders') || '[]'),
        productionTasks: JSON.parse(localStorage.getItem('f_estudio_db_tasks') || '[]'),
        transactions: JSON.parse(localStorage.getItem('f_estudio_db_transactions') || '[]'),
        alerts: JSON.parse(localStorage.getItem('f_estudio_db_alerts') || '[]'),
        okrs: JSON.parse(localStorage.getItem('f_estudio_db_okrs') || '[]'),
        lastSynced: new Date().toISOString()
      };

      // Since Apps Script requires POST redirect handling, we call fetch with no-cors or standard fetch
      // depending on Web App endpoint constraints.
      const response = await fetch(sheetsUrl, {
        method: 'POST',
        mode: 'no-cors', // standard approach for direct client-to-apps-script POST without CORS preflight
        headers: {
      ...
      // No-cors mode doesn't return full details, so we treat successful completion as synced!
      setSyncStatus('success');
      setSyncMessage(`¡Bases de datos cargadas con éxito en Google Sheets! Hora: ${new Date().toLocaleTimeString()}`);
      localStorage.setItem('f_estudio_last_cloud_sync', new Date().toLocaleString());
    } catch (err: any) {
      console.error(err);
      setSyncStatus('error');
      setSyncMessage(`Fallo en el puente: ${err.message || 'Verifica que la URL sea válida y esté activa.'}`);
    }
  };

  // Pulls state from Google Sheets (Cloud load)
  const handleLoadFromCloud = async () => {
    ...
    setSyncStatus('syncing');
    setSyncMessage('Descargando último estado consolidado desde Google Sheets...');
    ...
    try {
      ...
      if (data.status === 'empty') {
        setSyncStatus('error');
        setSyncMessage('La hoja de cálculo está conectada pero vacía. Haz una carga de datos primero.');
        return;
      }

      // Sync local storage and force reload page to sync React state
      if (data.fichasTecnicas) localStorage.setItem('f_estudio_db_fichas', JSON.stringify(data.fichasTecnicas));
      if (data.orders) localStorage.setItem('f_estudio_db_orders', JSON.stringify(data.orders));
      if (data.productionTasks) localStorage.setItem('f_estudio_db_tasks', JSON.stringify(data.productionTasks));
      if (data.transactions) localStorage.setItem('f_estudio_db_transactions', JSON.stringify(data.transactions));
      if (data.alerts) localStorage.setItem('f_estudio_db_alerts', JSON.stringify(data.alerts));
      if (data.okrs) localStorage.setItem('f_estudio_db_okrs', JSON.stringify(data.okrs));

      setSyncStatus('success');
      setSyncMessage('¡Datos descargados e importados al almacenamiento local con éxito! Recargando...');
      setTimeout(() => {
      ...
      console.error(err);
      setSyncStatus('error');
      setSyncMessage(`Error de descarga: ${err.message || 'La API devolvió un error CORS o de red.'}`);
    }
  };
  ...
  return (
    <div className="space-y-8 animate-fadeIn" id="module-direccion-view">
      ...
      {/* 1. Header & Google Sheets Integrator */}
      <div className="bg-slate-900 text-slate-100 p-6 rounded-2xl shadow-xl border border-slate-800 relative overflow-hidden" id="google-sheets-sync-bridge">
        <div className="absolute right-0 top-0 opacity-10 pointer-events-none transform translate-x-12 -translate-y-12">
          <Database size={220} className="text-white" />
        </div>
        <div className="flex flex-col lg:flex-row lg:items-center justify-between gap-6 z-10 relative">
          <div className="max-w-2xl">
            <div className="inline-flex items-center gap-1.5 bg-indigo-500/15 text-indigo-400 px-3 py-1 rounded-full text-[10px] font-bold uppercase tracking-wider mb-2 border border-indigo-500/30">
              <CloudLightning size={12} />
              <span>Conectividad de Base de Datos Cloud</span>
            </div>
            <h2 className="text-xl font-black text-white tracking-tight leading-none">
              Puente de Sincronización en la Nube (Google Sheets)
            </h2>
            <p className="text-xs text-slate-300 mt-2 leading-relaxed">
              Sincroniza en tiempo real este ERP con tu hoja de cálculo mediante un servicio seguro de Google Apps Script. 
              Esto te permite abrir y modificar el sistema desde múltiples dispositivos manteniendo una base de datos centralizada.
            </p>
          </div>
          ...
          <div className="flex flex-col sm:flex-row gap-3">
            <button
              ...
              className="px-4 py-2 text-xs font-bold bg-slate-800 text-slate-200 border border-slate-700 rounded-lg hover:bg-slate-700 transition-all flex items-center justify-center gap-1.5 cursor-pointer shadow-sm"
              id="btn-toggle-script-code"
            >
              <span>Generar Código Apps Script</span>
              <ChevronRight size={14} className={`transform transition-transform ${showAppsScriptCode ? 'rotate-90' : ''}`} />
            </button>
            <a 
              href="https://sheets.new" 
              target="_blank" 
              rel="noopener noreferrer"
              className="px-4 py-2 text-xs font-bold bg-indigo-600 text-white rounded-lg hover:bg-indigo-700 transition-all flex items-center justify-center gap-1.5 cursor-pointer shadow-md"
            >
              <span>Crear Google Sheet</span>
              <ExternalLink size={14} />
            </a>
          </div>
        </div>

        {/* Apps Script Code Instructions Drawer */}
        {showAppsScriptCode && (
          <div className="mt-6 p-5 bg-slate-950 border border-slate-800 rounded-xl space-y-4 animate-fadeIn" id="script-code-instructions">
            <div className="flex justify-between items-center">
              <span className="text-xs font-bold text-slate-400 uppercase tracking-wider">Instrucciones para Apps Script</span>
              <button 
                onClick={handleCopyCode}
                className="px-3 py-1 bg-indigo-500/10 text-indigo-400 hover:bg-indigo-500/20 text-xs font-extrabold rounded-md flex items-center gap-1.5 border border-indigo-500/20 transition-all"
                id="btn-copy-script"
              >
            ...
            <p className="text-[11px] text-slate-400 leading-relaxed">
              Copia el código a continuación, pégalo en el editor de Apps Script de tu Google Sheet (<strong>Extensiones &gt; Apps Script</strong>), guárdalo y haz clic en <strong>Implementar &gt; Nueva implementación</strong>. Elige el tipo de "Aplicación web", configura el acceso a "Cualquiera" y copia la URL de ejecución.
            </p>

            <pre className="text-[10px] bg-slate-900 text-slate-300 p-4 rounded-lg overflow-x-auto border border-slate-800 max-h-48 font-mono select-all">
        ...
        <div className="mt-5 pt-5 border-t border-slate-800 flex flex-col md:flex-row gap-4 items-center">
          <div className="w-full md:flex-grow">
            <div className="relative">
              <Database size={15} className="absolute left-3.5 top-1/2 transform -translate-y-1/2 text-slate-500" />
              <input
                type="text"
                value={sheetsUrl}
                onChange={(e) => handleSaveSheetsUrl(e.target.value)}
                placeholder="Pega la URL de tu Google Apps Script Web App (ej. https://script.google.com/macros/s/.../exec)"
                className="w-full bg-slate-950/70 text-xs text-slate-200 pl-10 pr-4 py-2.5 rounded-xl border border-slate-800 focus:border-indigo-500 outline-none placeholder-slate-600 transition-colors"
                id="input-sheets-url"
              />
            </div>
          </div>
          ...
          <div className="flex gap-2 w-full md:w-auto flex-shrink-0">
            <button
              ...
              className="flex-1 md:flex-initial bg-indigo-600 hover:bg-indigo-700 text-white text-xs font-extrabold px-4 py-2.5 rounded-xl flex items-center justify-center gap-1.5 transition-colors cursor-pointer shadow-md disabled:bg-slate-800 disabled:text-slate-500"
              id="btn-sync-cloud-post"
            >
              <RefreshCw size={13} className={syncStatus === 'syncing' ? 'animate-spin' : ''} />
              <span>Enviar a la Nube</span>
            </button>
            <button
              onClick={handleLoadFromCloud}
              disabled={syncStatus === 'syncing'}
              className="flex-1 md:flex-initial bg-slate-800 hover:bg-slate-700 text-slate-200 text-xs font-extrabold px-4 py-2.5 rounded-xl flex items-center justify-center gap-1.5 border border-slate-700 transition-colors cursor-pointer shadow"
              id="btn-sync-cloud-get"
            >
        ...
        {/* Sync Status Banner */}
        {syncMessage && (
          <div className={`mt-3.5 text-xs p-3 rounded-lg flex items-center gap-2 border ${
